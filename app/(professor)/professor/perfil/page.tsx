...
export default async function ProfessorPerfilPage() {
  ...
  const livrosConcluidos = livros?.filter((l) => l.status === "done").length ?? 0;

  return (
    <div className="max-w-2xl mx-auto animate-fade-in">
      <div className="mb-8">
        <h1 className="text-2xl font-bold text-jurua-dark">Meu Perfil</h1>
        <p className="text-muted text-sm mt-1">Gerencie suas informações e acesso</p>
      </div>
      ...
      <Card className="mb-6">
        ...
        <CardBody>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
            ...
            <div className="rounded-lg border border-border bg-jurua-cream p-3">
              <p className="text-muted text-xs mb-1">PDFs enviados</p>
              <p className="font-medium text-jurua-dark">
                {totalLivros} total · {livrosConcluidos} processados
              </p>
            </div>
          </div>
        </CardBody>
